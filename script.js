...
const productsFilters = document.getElementById(
	"products-filters"
);
const filtersBackdrop = document.getElementById(
	"filters-backdrop"
);
const closeFiltersBtn = document.getElementById(
	"close-filters"
);
...
if (closeFiltersBtn) {
	closeFiltersBtn.addEventListener(
		"click",
		closeFilters
	);
}
...
function openGumroadProductDialog(product) {
	...
	// Use the remaining HTML as the description
	document.getElementById(
		"dialog-description"
	).innerHTML = tempDiv.innerHTML;

	// Hide the separate features container as features are now part of the description
	const featuresContainer =
		document.getElementById(
			"dialog-features-container"
		);
...
function displayBlogPosts(page) {
	...
	const postsToDisplay = allBlogPosts.slice(
		startIndex,
		endIndex
	);
